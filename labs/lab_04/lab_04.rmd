...
**A2. All of the top ten spenders were either candidate committees or PACs. A number of the top 10 were "friends of" accounts, which are used for a campaign, so this money was likely spent on Facebook ads to promote their candidacy. Some were running for Maryland slate, which is for the positions of governor and lieutenant governor. After looking through the ad library, only some of these ads provide an estimated audience size and the amount spent on the ad. For most campaigning for governor, their ads stopped around the end of July, but were most expensive at the end of June and early July.**
...
**A3. Wes Moore and Tom Perez are significantly missing from that list. Wes Moore and Tom Perez did run Facebook ads, so why don't their names pop up? The biggest recipients of media expenses by Wes Moore were SDSK, Foglamp, and Apollo Artistry.**

```{r}
moore_media <- media_expenses_clean |>
...
**A4. There are some similarities in the top 10 for each table, like Casa in Action PAC, but for the most part, they are very different. Friends of Jain Ashwani spent all their money on Facebook ads, as well as Franchot Anderson-Walker for Maryland Slate, and Friends of Salimah Jasani.**

```{r}
online_ad_spending <- media_expenses_clean |>
  filter(str_detect(expense_purpose, "Online")) |>
  group_by(committee_name) |>
  summarize(total_online_spent = sum(amount, na.rm = TRUE)) |>
...
**A5 Maryland campaign spending records show that several political candidates, committees, and campaigns spent significant amounts of money on Facebook/Meta during the 2022 primary. Through standardizing vendor names for Facebook and Meta, it's possible to identify which groups relied most heavily on the social media platform for advertising. You can examine how heavily Maryland campaigns relied on Facebook for advertising compared to other online forms, or even through news organizations and traditional media. **
