...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer: object id, zipcode2 , zip name, existing, zipcode1, shape length, shape area, geometry**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer: The code just shows you the data that we ran above**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This shows the state of Maryland, with each zip code, along with latitude and longitude lines and coordinates.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: allows for shapefiles to be downloaded and loaded into R.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The table shows us counties in the U.S., with shape files for each.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the information and shapefile for each county in Maryland.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the outline of each county, but isn't entirely accurate because the Chesapeake Bay is no longer visible.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Overlaps the two maps above, but the waterways are still less visible.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the foreclosure count in each zip code for each month from July 2021 to March 2023.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Only shows the foreclosures in each zip code in Maryland in the month of March 2023.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This map color codes each zip code. So for zips with a lower number of foreclosures, the area will be a darker blue. For zips with higher numbers of foreclosures, the area will be a lighter blue.**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer: Libraries are run to create maps and use Census data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the notices of foreclosure for each county in Maryland, along with timestamps.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Shows the population of each county in Maryland, along with the shapefile for each, and the GEOID.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This just gives you a glimpse of both data tables.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The data table now shows just the county name, the population, and the shapefile for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: The two tables are joined together, and now the md_pop_with_foreclosure has information about county, timestamps, number of notices, and total population for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: A rate column has been added to the data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: This map is labeled based on rate of foreclosures. So for counties with higher rates of foreclosure, the county shape will be shaded in with a lighter blue. If the rate is lower, the shape will be shaded in with a darker blue. For example, Charles County has the highest rate of foreclosures, which is 2.03.**
...
**Task** Run the following code. Describe the output in the space below. **Answer: Now, there is more color variation for rates, with the highest rates in yellow and the lowest in black. Purple, pink and orange are rates that range from 0.5 to 2.0.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer: For the most part, the state of Maryland doesn't have many foreclosures. It seems like counties that border major cities, like Washington D.C. or Baltimore, have the lowest or second lowest rates of foreclosure, but the cities have higher rates. Does foreclosure have to do with median income for each county? That could be something else to test out with code.**
