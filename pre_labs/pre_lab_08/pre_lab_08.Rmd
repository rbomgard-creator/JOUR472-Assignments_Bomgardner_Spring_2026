...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer: Mutate creates a new column called month, which pulls out the date column to do so. The group by organizes the data so that all the rows with the same month are grouped together. The summarize counts how many rows, or calls, are in each month group, giving us calls per month. Then the arrange function sorts the data from highest number of calls to lowest. And finally the last line of code shows us a pop-up window of the rest of the code listed.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A big blank grey square.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: A line graph with the amount of calls per month.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: This graph has the data in ascending order, with the lowest calls per month on the left, and an increase from then on.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: The coordinates have been flipped, which is what the coord_flip() is for.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: There is no longer a grey grid, it's now white.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: The background is blue, the theme has changed.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: There is a title and a source.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: The date and total calls for the day are shown, starting in February of 2022 and ending in January 2023.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: This graph looks like a heart monitor, I'm not going to lie. But you can see the spikes and other outliers better with this graph than the one used above.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: The dates are labeled at the bottom, but they're hard to see because they're all squeezed together.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: The dates have been turned to a 45 degree angle, but they are still overlapping and hard to see.**
