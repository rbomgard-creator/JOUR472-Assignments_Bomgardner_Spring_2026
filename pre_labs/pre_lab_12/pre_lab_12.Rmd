...
**Task** Run the following code and describe the dataframe it outputs. **Answer: A list of doctors and complaints against them, including specific dates, letters from the board, etc.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer: takes our repeat words**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer: Takes every word used in the database, regardless of repeats. Removed punctuation, made text lowercase.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer: Not really, because the word "the" is used all the time, regardless of employment status.**
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer: It's better, but most of these words are expected. Interesting words include consent, suspend, code, records, which could point towards a doctor's outcome.**
...
**Task** Run the following code and describe the results. **Answer: The words used in both years are very similar, but the distribution is a little bit different.**
...
**Task** Run the following code and describe the results. **Answer: These phrases are more informative of what each letter might be about or what each doctor is accused of.**
...
**Task** Run the following code and describe the results. **Answer: Similar to 2021, just in a different order.**
...
**Task** Run the following code and describe the results. **Answer: Similar to 2021 and 2023, just in a different order.**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer: Issues or issue could just mean that the Board is issuing a certain statement to the doctor, or that a certain issue regarding a law is important for the letter. This doesn't mean that these two words are negative, they could just be used as a verb without looking at the connotation. Also the word object cannot be considered negative because it depends on what the context is. Are we talking about an object being used or is a doctor objecting to something?**
