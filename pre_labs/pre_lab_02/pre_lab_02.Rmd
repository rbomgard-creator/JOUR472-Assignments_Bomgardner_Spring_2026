...
**Answer: The original dataset will stay unchanged, so that we don't lose other columns. You can come back to the original table without anything being changed and you can now play around with this new variable in whatever way you want.**
...
**Answer:Instead of "3/12/2021," we now see "2021-03-12." The lubridate function has now changed the expenditure_date column to a date datatype instead of chr. **
...
**Answer: So even though the date says 2022-06-01, that represents a whole month, not just that day. So this means for the month of June 2022, 23102549.08 dollars were spent.**
...
**Answer: This code adds two more columns to the table, called total_votes and pct_moore. The total_votes column is made by adding up the votes for each candidate, including write-ins. The pct_moore column is made by dividing all of Wes Moore's votes by the total votes. So you need the new total_votes column to make the pct_moore column.**
...
**Answer: Nothing changed, expect the order that the data was presented to us. Now, you can see which Moore's percentage for each county from largest to small. That's what the arrange(desc()) does. So we can see that 89% of Prince George's County voters chose Moore to be their governor in 2022.**
...
**Answer: Yes and no. It's surprising to see that only 22% of Garrett County's voters picked Wes Moore, just because that number is so low. But descending order shows us that almost the entirety of Prince George's County voters want Moore as their governor. I think that's a better story.**
...
**Answer: str_to_lower: code is transformed to all lowercase, str_to_title: only the first letter of each word is uppercase.**
...
**Answer: The mutate statement makes a new column called state within our table. Case_when and str_detect are used on the address column to see if a match of the below states is found and then assigns the corresponding state an abbreviation in the state column.**
...
**Answer: Yes, it means that each of these states has spent the total listed next to each abbreviation.**
...
1.  Which UMD courses have more than 50 seats available? What departments are they in? (umd courses)

2.  Are their certain years with higher spending than usual? (md expenditures)

3.  Which payees received the most total money after standardizing their names to uppercase? (md expenditures)
