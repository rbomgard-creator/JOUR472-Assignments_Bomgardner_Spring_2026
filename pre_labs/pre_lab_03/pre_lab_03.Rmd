...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: "Vendor Name" is a chr data type, while "Amount" is a dbl data type.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer: 553 zip codes don't have five characters. This could be because the data is being read as a number and not a character, so it could have taken off important zeros, or because they are country codes.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer: NA rows in the "Category" column.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The smallest amount for a grant is 60 dollars for Land Preservation Trust, Inc. Why is it so small? Who doesn't have 60 dollars?**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer: The headers are now able to be easily read by both the computer and by us. They're now lowercase and have no spaces.**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer: We are choosing the cleaned_md_grants_loans table and cleaning the names of source, grantee and description. In this case, the mutate function, along with str_to_upper, changes the data in these columns to be all uppercase.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: There are 4 duplicates among BOARDS AND COMMISSIONS/GOVERNOR'S OFFICE OF CRIME CONTROL AND PREVENTENTION, all with the same fiscal year, zip code, amount, and grantee.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: 19,453 rows instead of the original 19,482.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: Extensions of zip codes are no longer visible, only the original five digits of regular zip codes.**
