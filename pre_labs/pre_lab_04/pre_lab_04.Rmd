...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: Different "inc" being used, shortening of words like "association", etc.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: Some grantees' names were completely capitalized or certain punctuation was removed.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer: Almost every grantee name was changed to completely capitalized. Grantee names that were similar to another are now listed with the same name. For example, Walden Sierra was listed like normal and also like Walden Sierra Inc. In clean names, it's listed as Walden Sierra, Inc. Any spelling mistakes or accidentally capitalized letters are now fixed.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: Some of them are changed, but n_gram seems to be more lax with changes. Most of the changes are similar to the original grantee name.**
