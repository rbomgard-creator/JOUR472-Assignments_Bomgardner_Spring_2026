...
**Answer: observations - 79366, variables - 9.**
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: The datatype for the "id" column is chr, or characters. For "seats," the datatype is num, or numbers (numerical data). Also displayed for "seats" is dbl, or double, which is also a numerical datatype.**
...
**Answer: Take umd_courses and then group each term by how many classes are offered.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Terms are arranged in descending order, so now it's easier to see which term had the largest number of classes offered and which has the smallest.**
...
**Answer: department was added in the group_by code line, which added another column, but also showed how many classes were offered each term for each department.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Only numerical data is shown here, no departments. Total seats are added. This table shows terms and how many classes were offered, along with total number of seats.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: Mean and median functions are added to the code, which creates two more columns to the table. Mean and median seats have similar values as you go down each row.**
...
**Answer: Could there be a class with 0 seats? Minimum and maximum seats are added to the code. Two more columns are added to the table. The data is still sorted by descending through the total seats column.**
...
1.  Are there classes at UMD that actually have 2500 seats? Or is that number so large due to online classes or multiple sections and multiple instructors of the same class?

2.  Would there be a better way to manage the term than by code? Maybe using more chr datatype than num?

3.  Why would there be 50 different syllabus listed for one course that only has 10 seats?
